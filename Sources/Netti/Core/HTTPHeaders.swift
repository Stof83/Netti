...
extension HTTPHeader {
    ...
    ///
    /// NetworkingKit offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// NetworkingKit offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// NetworkingKit provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All NetworkingKit `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not
...
extension HTTPHeaders {
    /// The default set of `HTTPHeaders` used by NetworkingKit. Includes `Accept-Encoding`, `Accept-Language`, and
    /// `User-Agent`.
...
extension HTTPHeader {
    /// Returns NetworkingKit's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns NetworkingKit's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns NetworkingKit's default `User-Agent` header.
    ///
    /// See the [User-Agent header documentation](https://tools.ietf.org/html/rfc7231#section-5.5.3).
    ///
    /// Example: `iOS Example/1.0 (org.NetworkingKit.iOS-Example; build:1; iOS 13.0.0) NetworkingKit/5.0.0`
